...
async function readSkills(print = false) {
	...
	for (const row of sfia || []) {
		...
		function synthesizeDescription(level) {
			...
			const levelMeta = sfiaLevelByNumber.get(level);
			const levelLabel = levelMeta ? `${level} • ${levelMeta.level_name}` : `Level ${level}`;
			...
			return `AI created: ${skillName} — ${levelLabel}. ${context}.`;
		}
		...
		for (let level = 1; level <= 7; level++) {
			...
			if (!desc) {
				...
				desc = typeof aiDesc === 'string' && aiDesc.trim() ? aiDesc.trim() : synthesizeDescription(level);
			}
			levels.push({
				level,
