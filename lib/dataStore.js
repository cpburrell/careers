...
async function readSkills(print = false) {
	if (getDataSource() === 'db') {
		skills = await readSkillsFromDb();
	} else {
		const raw = await fs.readFile('./skills.json', 'utf8');
...
async function loadCore({ validate = true, strictValidation = true } = {}) {
	await Promise.all([readSkills(), readRoles()]);
	if (validate) validateData({ strict: strictValidation });
}

async function loadAll({ validate = true, strictValidation = true } = {}) {
	await Promise.all([readSkills(), readRoles(), readSFIACSV()]);
	if (validate) validateData({ strict: strictValidation });
}

async function readSkillsFromDb() {
	return withPgClient(async (client) => {
		const categoriesRes = await client.query('SELECT id, name FROM careers.categories ORDER BY id');
		const levelsRes = await client.query(
			'SELECT level, level_name, level_description FROM careers.sfia_levels ORDER BY level'
		);
		const skillsRes = await client.query('SELECT id, name, category_id FROM careers.skills ORDER BY id');
		const skillLevelsRes = await client.query(
			'SELECT skill_id, level, brief_description, full_description FROM careers.skill_levels ORDER BY skill_id, level'
		);

		const levelsBySkillId = Object.create(null);
		for (const row of skillLevelsRes.rows) {
			if (!levelsBySkillId[row.skill_id]) levelsBySkillId[row.skill_id] = [];
			levelsBySkillId[row.skill_id].push({
